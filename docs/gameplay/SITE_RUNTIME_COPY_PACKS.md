...
| `site.wikiwhy.secureStatus` | `SOURCE CHECKS RESTORED` |
...
Use `WIKIWHY SECURED` as the payoff heading (`site.wikiwhy.secure.title`). The
in-page status is `SOURCE CHECKS RESTORED`; the Recovery Map tile uses
`EVIDENCE CONNECTED`. These are separate UI slots, not competing statuses.
...
- `Result 4: Same cache, borrowed source badge`
...
- Use generated site boards only as storyboard/preview references. Production
  identity and state direction come from `TEN_SITE_PRODUCTION_INDEX.md`, the
  original mark manifests, and the exact site build briefs.
