...
- `marks/` contains original production marks for nine sites; ThreadIt's mark
  lives in its focused pack and is linked from the marks manifest.
...
chrome. Generated boards remain composition/story references; meaningful
runtime trees, statuses, controls, and evidence content must be DOM-native.

The authoritative all-site review is
[`TEN_SITE_PRODUCTION_INDEX.md`](../design/TEN_SITE_PRODUCTION_INDEX.md) and its
clean
[`ten-site-production-overview.svg`](../../apps/internet-recovery/art/site-assets/ten-site-production-overview.svg).

## Purpose

This is a design-only asset-use brief for the builder. It separates production
assets from storyboard/comparison history and explains how to crop the
production character sheets without regenerating new art for every message.
...
may be inspected for history. When a preview needs a storyboard crop, use the
indexed `*-rogue-ai-campaign.png` reference; runtime direction still comes from
the production index, exact brief, copy IDs, and original marks.
...
- use the indexed storyboard reference as a reduced preview image if helpful;
...
- If a preview uses generated art, does it use the indexed current storyboard
  reference while keeping identity, copy, and state logic production-native?
