...
- `apps/internet-recovery/art/characters/README.md` - production character
  sheet panel IDs.
- `docs/design/TEN_SITE_PRODUCTION_INDEX.md` and
  `apps/internet-recovery/art/site-assets/ten-site-production-overview.svg` -
  authoritative all-site contracts and clean visual review.
- `apps/internet-recovery/art/concepts/sites/README.md` - storyboard-reference
  set and superseded-history notes; not runtime identity or state input.
...
Use `amy_evidence` only. Reserve `amy_supportive` for hub and secured-site
encouragement.
...
- `amy_supportive` - hub support and secured-site encouragement
- `amy_evidence` - timestamps, source checks, warnings
- `amy_tools` - Shield instruction and system/tool explanation
