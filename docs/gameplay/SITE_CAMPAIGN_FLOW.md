# Internet Recovery OS site campaign flow
...
percentage points and reveals a new stable-looking page region. The exact jump
reflects overall session evidence, so strong reading can reach the story turn in
fewer passages without an explicit level-skip announcement.

The game does not show `passage 3 of 7`, because the precise Act I length can
adapt. It shows meaningful site state instead:
...
### Adaptive length without punishment

Act I length may respond to completion evidence, but it must not turn mistakes
into an obvious workload penalty.

- Strong, independent completion produces jumps near the top of the 10–20% band
  and can reach the story turn in about four or five Act I passages.
- Developing completion produces smaller but still meaningful jumps near the
  bottom of the band, resulting in more fresh passages with better support.
- Recognition uncertainty by itself never adds work.
- Added passages use different, worthwhile content; they do not repeat the same
  text or revoke visible repairs.
- The cap is fixed before play. Randomness can vary the route within the cap, not
  create an endless campaign.
- The player-facing fiction is that WikiWhy needs more recovered evidence, not
  that Finn failed.

The adaptation belongs to game/content rules and consumes theme-neutral session
outcomes. The Reading Engine does not decide campaign length.

### Dynamic progress reward

Progress is continuous rather than a binary skip/no-skip reward. After every Act
I passage, the site gains approximately 10–20% stability. The exact increase
uses balanced evidence rather than rewarding speed alone:

- the passage is substantially completed;
- accuracy is strong enough that the result is trustworthy;
- any independent comprehension check shows understanding;
- recognizer uncertainty is low enough to make the decision fair;
- faster pace may contribute, but there is no maximum-WPM penalty and speed
  cannot compensate for missing comprehension.

The player sees the repair wipe travel farther after a particularly strong
session, but the game does not expose a formula, grade the size of the jump, or
announce that a level was skipped. Reward copy stays vague and diegetic:
...
Every accepted passage earns at least a meaningful 10% advance. Recognition
uncertainty cannot reduce the jump, and a smaller jump is never labeled as a
penalty. The next worthwhile passage simply opens as ordinary recovery. The
system clamps the final Act I jump to exactly 80%, then triggers the story turn;
it does not overshoot into the finale.
...
- a repaired citation briefly changes behind the wipe;
- an edit timestamp is newer than Finn's repair;
- Techno stops playing with her ball and watches a blinking background process;
- the status changes from **MOSTLY STABLE** to **VERIFYING CHANGES**.
...
At 100% shield stabilization:

- Amy says, “Great. The changes are sealed. This one stays fixed.”
