...
function destinationOverlay(state) {
  ...
  const mark = failed ? "×" : "○";
  const footer = failed ? `<text x="716" y="263" class="mg-overlay-foot" text-anchor="middle" fill="${COLORS.corruption}">DIDN'T LOCK · CONTINUE</text>` : "";
  return `<g data-module="moving-target-overlay" data-purpose="repair-target" data-attempt-state="${state.overlay}"><rect x="554" y="128" width="330" height="${failed ? 150 : 124}" rx="10" fill="#FAF8F1" stroke="${COLORS.repair}" stroke-width="4"/><rect x="554" y="128" width="330" height="44" rx="10" fill="${COLORS.repair}"/><rect x="554" y="160" width="330" height="12" fill="${COLORS.repair}"/><text x="570" y="158" class="mg-lock-title">LOCK IN THE REPAIR</text><rect x="570" y="190" width="34" height="34" rx="6" fill="${itemFill}" stroke="${itemStroke}" stroke-width="2"/><text x="587" y="215" class="mg-lock-mark" text-anchor="middle" fill="${COLORS.corruption}">${mark}</text><text x="622" y="202" class="mg-overlay-head" fill="${COLORS.corruption}">GO DIRECTLY TO</text><text x="622" y="224" class="mg-overlay-head" fill="${COLORS.corruption}">THE LIBRARY</text>${footer}</g>`;
}
