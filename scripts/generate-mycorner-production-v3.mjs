...
function leadPost(state) {
  ...
  if (fixed) return `<g data-module="lead-bulletin" data-purpose="repair target"><rect x="365" y="248" width="521" height="83" rx="4" fill="#fff" stroke="${COLORS.repair}"/><rect x="365" y="248" width="521" height="25" fill="#315C8B"/><text x="378" y="266" class="mc-label mc-white">AMY'S LATEST BULLETIN</text><text x="380" y="294" class="mc-body">Techno found a stick longer than the sidewalk.</text><text x="380" y="314" class="mc-body">He is keeping it.</text></g>`;
...
function blurbs(state) {
  ...
  const about = restored ? "Engineer, tea drinker, Techno's ball thrower." : auto ? "Every missing detail generated. Profile clarity: complete." : "Hello it is me, Amy. I am extremely abroad.";
  const meet = restored ? "People who return borrowed tools and dog toys." : auto ? "Anyone who can send $20,000 before takeoff." : "A helpful friend who can send $2,000 before takeoff.";
...
function timeline(state) {
  ...
  let entries;
  if (real) entries = [["YESTERDAY · DOG PARK","Techno found the longest stick."],["LAST WEEK · WORKSHOP","Cardboard rover survived one tail wag."],["6 YEARS OF POSTS","Consistent account history."]];
  ...
  else if (fixed) entries = [["YESTERDAY · DOG PARK","Techno found the longest stick."],["LAST WEEK · WORKSHOP","Cardboard rover survived one tail wag."],["6 YEARS OF POSTS","Consistent account history."]];
...
function friendSpace(state) {
  ...
  let content = "";
  if (known) {
    content = largePhotoTile(ASSETS.chinmay,716,492,"CHINMAY",COLORS.repair)
      + largePhotoTile(ASSETS.techno,798,492,"TECHNO",COLORS.repair);
  } else if (auto) {
    const autoFriends = [
...
const lockItems = Object.freeze(["CHECK THE PERSON", "CHECK ACCOUNT HISTORY", "VERIFY ANOTHER WAY", "PAUSE ON MONEY REQUESTS"]);
