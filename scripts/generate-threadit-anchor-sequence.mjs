...
function collapsedCopies(finalized = false) {
  const rows = [
    ...
    ["u/raw_fish_fan_2", "Every kind of raw fish is ALWAYS safe.", "3 DUPLICATES", "−42", "down"],
  ];
...
function sideRail(state) {
  ...
  const posts = auto
    ...
    : fixed
      ? [["u/food_safety_guide", "↑ 2.4K · current guidance"], ["u/kitchen_coach", "↑ 284 · handling details"], ["u/just_asking", "↑ 94"]]
...
function footer(state) {
  ...
  return `<g data-module="site-progress" data-purpose="progress-only"><rect x="109" y="677" width="802" height="161" fill="#F7F5EE"/><line x1="109" y1="677" x2="911" y2="677" stroke="#8E9AA0"/><text x="126" y="716" class="thread-meter" fill="${color}">${label}</text><text x="284" y="716" class="thread-meter" fill="${color}">${state.progress}%</text><rect x="126" y="732" width="752" height="25" fill="url(#threadRedHatch)" stroke="${color}"/><rect x="126" y="732" width="${fill}" height="25" fill="${color}" data-role="site-progress-fill" data-percent="${state.progress}"/></g>`;
}
...
function checklist(state) {
  ...
  return `<g data-lock-overlay="true"><rect x="500" y="356" width="330" height="166" rx="10" fill="#FAF8F1" stroke="${COLORS.repair}" stroke-width="3"/><rect x="500" y="356" width="330" height="40" rx="10" fill="${COLORS.repair}"/><rect x="500" y="383" width="330" height="13" fill="${COLORS.repair}"/><text x="520" y="384" class="lock-title">LOCK IN THE REPAIR</text>${lockItems.map((item, index) => { const done = index < state.checklist; const y = 424 + index * 34; return `<rect x="524" y="${y - 22}" width="27" height="27" rx="5" fill="${done ? COLORS.repair : COLORS.corruptionSoft}" stroke="${done ? COLORS.repair : COLORS.corruption}"/><text x="537.5" y="${y - 3}" class="lock-mark" text-anchor="middle" fill="${done ? "#fff" : COLORS.corruption}">${done ? "✓" : "○"}</text><text x="563" y="${y}" class="lock-label" fill="${done ? COLORS.repairDark : COLORS.corruption}">${item}</text>`; }).join("")}</g>`;
}
