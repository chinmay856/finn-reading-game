...
function checklist(state) {
  ...
  return `<g data-lock-overlay="true"><rect x="390" y="326" width="490" height="310" rx="10" fill="#FAF8F1" stroke="${COLORS.repair}" stroke-width="3"/><rect x="390" y="326" width="490" height="56" rx="10" fill="${COLORS.repair}"/><rect x="390" y="365" width="490" height="17" fill="${COLORS.repair}"/><text x="415" y="362" class="lock-title">LOCK IN THE REPAIR</text>${lockItems.map((item, index) => { const done = index < state.checklist; const y = 431 + index * 51; return `<rect x="421" y="${y - 21}" width="29" height="29" rx="5" fill="${done ? COLORS.repair : COLORS.corruptionSoft}" stroke="${done ? COLORS.repair : COLORS.corruption}"/><text x="435" y="${y}" class="lock-mark" text-anchor="middle" fill="${done ? "#fff" : COLORS.corruption}">${done ? "✓" : "○"}</text><text x="468" y="${y}" class="lock-label" fill="${done ? COLORS.repairDark : COLORS.corruption}">${item}</text>`; }).join("")}</g>`;
}
