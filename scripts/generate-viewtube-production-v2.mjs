...
function titlebarPatch() {
  return `<g data-shared-shell-patch="site-identity"><rect x="112" y="24" width="520" height="29" fill="url(#titleGradient)"/><text x="126" y="46" class="window-title">www.viewtube.com</text><rect x="112" y="861" width="188" height="31" fill="url(#buttonGradient)" stroke="#6d6d67" stroke-width="1.3"/><text x="54" y="882" class="task-label" text-anchor="middle">START</text><text x="146" y="882" class="task-label">VIEWTUBE</text></g>`;
}
...
function checklist(state) {
  ...
  return `<g data-lock-overlay="true"><rect x="380" y="292" width="506" height="388" rx="10" fill="#FAF8F1" stroke="${COLORS.repair}" stroke-width="3"/><rect x="380" y="292" width="506" height="56" rx="10" fill="${COLORS.repair}"/><rect x="380" y="332" width="506" height="16" fill="${COLORS.repair}"/><text x="404" y="329" class="lock-title">LOCK IN THE REPAIR</text>${locks.map((label, index) => { const done = index < state.checklist; const y = 400 + index * 53; return `<rect x="410" y="${y - 23}" width="29" height="29" rx="5" fill="${done ? COLORS.repair : COLORS.corruptionSoft}" stroke="${done ? COLORS.repair : COLORS.corruption}"/><text x="424.5" y="${y - 3}" class="lock-mark" text-anchor="middle" fill="${done ? "#fff" : COLORS.corruption}">${done ? "✓" : "○"}</text><text x="461" y="${y}" class="lock-label" fill="${done ? COLORS.repairDark : COLORS.corruption}">${label}</text>`; }).join("")}</g>`;
}
