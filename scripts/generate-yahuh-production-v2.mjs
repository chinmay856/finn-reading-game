...
function checklist(state) {
  ...
  return `<g data-lock-overlay="true"><rect x="409" y="344" width="455" height="250" rx="10" fill="#FAF8F1" stroke="${COLORS.repair}" stroke-width="3"/><rect x="409" y="344" width="455" height="56" rx="10" fill="${COLORS.repair}"/><rect x="409" y="383" width="455" height="17" fill="${COLORS.repair}"/><text x="434" y="380" class="lock-title">LOCK IN THE REPAIR</text>${lockItems.map((item, index) => { const done = index < state.checklist; const y = 449 + index * 57; return `<rect x="440" y="${y - 23}" width="31" height="31" rx="5" fill="${done ? COLORS.repair : COLORS.corruptionSoft}" stroke="${done ? COLORS.repair : COLORS.corruption}"/><text x="455" y="${y}" class="lock-mark" text-anchor="middle" fill="${done ? "#fff" : COLORS.corruption}">${done ? "✓" : "○"}</text><text x="490" y="${y}" class="lock-label" fill="${done ? COLORS.repairDark : COLORS.corruption}">${item}</text>`; }).join("")}</g>`;
}
