...
function checklist(state) {
  ...
  return `<g data-lock-overlay="true"><rect x="520" y="356" width="330" height="166" rx="10" fill="#FAF8F1" stroke="${COLORS.repair}" stroke-width="3"/><rect x="520" y="356" width="330" height="40" rx="10" fill="${COLORS.repair}"/><rect x="520" y="383" width="330" height="13" fill="${COLORS.repair}"/><text x="540" y="384" class="lock-title">LOCK IN THE REPAIR</text>${lockLabels.map((label, index) => { const done = index < state.checklist; const y = 424 + index * 34; return `<rect x="544" y="${y - 22}" width="27" height="27" rx="5" fill="${done ? COLORS.repair : COLORS.corruptionSoft}" stroke="${done ? COLORS.repair : COLORS.corruption}"/><text x="557.5" y="${y - 3}" class="lock-mark" text-anchor="middle" fill="${done ? "#fff" : COLORS.corruption}">${done ? "✓" : "○"}</text><text x="583" y="${y}" class="lock-label" fill="${done ? COLORS.repairDark : COLORS.corruption}">${label}</text>`; }).join("")}</g>`;
}
