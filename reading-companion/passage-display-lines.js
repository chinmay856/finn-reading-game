...
function splitSentences(paragraph) {
  ...
  if (sentenceSegmenter) {
    return [...sentenceSegmenter.segment(protectedText)]
      .map(({ segment }) => segment.replaceAll("\u0000", ".").replaceAll("\u0001", "?").replaceAll("\u0002", "!").replaceAll("\u0003", ".").replace(/\[(\d+)\]\./gu, ".[$1]").trim())
      .filter(Boolean);
  }
  return (protectedText.match(SENTENCE_PATTERN) ?? [protectedText])
    .map((sentence) => sentence.replaceAll("\u0000", ".").replaceAll("\u0001", "?").replaceAll("\u0002", "!").replaceAll("\u0003", ".").replace(/\[(\d+)\]\./gu, ".[$1]").trim())
    .filter(Boolean);
}
