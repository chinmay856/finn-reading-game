...
test("ViewTube exposes seven candidate playtests without promoting content", () => {
  const first = selectNextViewTubePassage({ completedPassageIds: [] }, { lane: "playtest" });
  ...
  assert.equal(first.passage?.id, VIEWTUBE_DECK_A_IDS[0]);
  assert.equal(first.passage?.availability, "candidate");
  const next = selectNextViewTubePassage({ completedPassageIds: [first.passage.id] }, { lane: "playtest" });
  assert.equal(next.passage?.id, VIEWTUBE_DECK_A_IDS[1]);
