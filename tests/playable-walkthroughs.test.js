...
const expectations = Object.freeze({
  ...
  mycorner: Object.freeze({ count: 8, first: 4, pages: [2, 3, 4, 5, 8, 9, 10, 11], secured: "_p12.png" }),
  ...
  "spotty-fi": Object.freeze({ count: 9, first: 5, pages: [2, 3, 4, 5, 6, 9, 10, 11, 12], secured: "_p13.png" }),
  ...
  for (const mission of Object.values(PLAYABLE_WALKTHROUGHS)) {
    for (const passage of mission.passages) {
      ...
      if (passage.sourceDocumentId) {
        assert.equal(wordCount, passage.spokenWordCount, `${passage.id}: preserve reviewed length`);
      } else if (passage.reviewStatus.startsWith("human-reviewed-frozen-")) {
        assert.ok(wordCount >= 180 && wordCount <= (passage.id === "wikiwhy-07" ? 400 : passage.id === "wikiwhy-09" ? 340 : 325), `${passage.id}: ${wordCount} words`);
...
test("MyCorner preserves the reviewed twelve-state identity-check sequence", () => {
  ...
  assert.deepEqual(mission.passages.map(({ id }) => id), [
    "mycorner-01", "mycorner-02", "mycorner-03", "mycorner-04", "mycorner-05",
    "mycorner-06", "mycorner-07", "mycorner-09",
  ]);
